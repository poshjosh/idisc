...
import com.idisc.web.FeedComparatorUserHitsOnSiteImpl;
...
public class Feeds extends Selectfeeds {
  ...
  private transient static final Logger LOG = Logger.getLogger(Feeds.class.getName());
  ...
  public synchronized List<Feed> select(HttpServletRequest request)
    ...
    LOG.entering(this.getClass().getName(), "#select(HttpServletRequest)");
...
//    }
  ...
  protected String[] getSearchTerms(HttpServletRequest request) throws ValidationException {
    if(_$st == null) {
        final String [] queries =  super.getSearchTerms(request);
        final List<String> queryList = queries == null || queries.length == 0 ? 
                Collections.EMPTY_LIST : Arrays.asList(queries);
        final List<String> all = this.composeSearchTerms(request, queryList);
        _$st = all.isEmpty() ? new String[0] : all.toArray(new String[0]);
    }
    return _$st;
  }

  protected List<String> composeSearchTerms(HttpServletRequest request, List<String> searchTermList) 
      throws ValidationException {
    final int max = this.getSearchTermsMax(request);
    final List<String> output;
    if(searchTermList.isEmpty() && this.getSearchTerm(request) != null) {
      output = Collections.EMPTY_LIST;
    }else{
      if(searchTermList.size() >= max) {
        output = searchTermList;
      }else {
        output = new ArrayList<>();
        output.addAll(searchTermList);
        this.addPreferred(request, output, max);
      }  
    }
    return output.size() <= max ? output : output.subList(0, max);
  }
  
  protected void addPreferred(HttpServletRequest request, List<String> all, int max) {
    if(all.size() < max) {
  ...
  private List<Feed> filter(List<Feed> list, Predicate<Feed> test, String testName,
    ...
    LOG.log(debug ? Level.INFO : Level.FINE,
          ...
          if(comparator instanceof AutoCloseable) {
            try{
              ...
              if(LOG.isLoggable(Level.WARNING)){
                     LOG.log(Level.WARNING, "Exception closing: "+comparator, e);
              }
            }
          }
        }
...
LOG.log(debugTimeAndMemory ? Level.INFO : Level.FINE,
  ...
  protected Comparator<Feed> getComparator(AppContext appContext, boolean reverseOrder) {
    ...
    switch(type) {
      case "userSiteHitcount":
        output = new FeedComparatorUserHitsOnSiteImpl(appContext, this.getInstallation(), reverseOrder);
        break;
      default:
        output = new DefaultFeedComparator(reverseOrder);
        break;
    }
    return output;
  }
