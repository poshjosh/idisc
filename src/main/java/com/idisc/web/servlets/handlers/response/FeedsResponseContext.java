...
public class FeedsResponseContext<E extends Feed> extends SuccessHandlerContext<List<E>> {
  ...
  private transient static final Logger LOG = Logger.getLogger(FeedsResponseContext.class.getName());
...
LOG.log(Level.FINER, "Installation: {0}, number of notifications: {1}", new Object[]{ installation,  notifications.size()});
}
    
    final Map commentNotices = installation == null ? Collections.EMPTY_MAP : getCommentNotices(installation, feeds);
    
if(LOG.isLoggable(Level.FINER)){
LOG.log(Level.FINER, "Number of commentNotices: {0}", new Object[]{ installation,  commentNotices.size()});
}
    
if(LOG.isLoggable(Level.FINER)){
LOG.log(Level.FINER, "Hot news feedids: {0}", new Object[]{ installation,  hotnewsFeedids});
}
...
if(LOG.isLoggable(Level.FINER)){
LOG.log(Level.FINER, "Installation: {0}, number of feeds: {1}", new Object[]{ installation,  feeds==null?null:feeds.size()});
}
  ...
  public Collection<Feed> getNotices(List<E> feeds, Collection<Feed> defaultValue) {
    ...
    try {
      ...
      if (notices != null && !notices.isEmpty()) {
        ...
        if(LOG.isLoggable(Level.FINER)){
            LOG.log(Level.FINER, "Adding {0} notices to output", notices.size());
        }
        
        com.idisc.pu.FeedDao feedService = new DefaultFeedService(this.getAppContext());
//                new com.idisc.pu.FeedDao(this.getAppContext().getIdiscApp().getJpaContext());
      ...
      if(LOG.isLoggable(Level.WARNING)){
         LOG.log(Level.WARNING, "Error loading notices", e);
      }
    }
