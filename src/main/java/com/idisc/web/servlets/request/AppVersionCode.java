...
public class AppVersionCode {
  ...
  private transient static final Logger LOG = Logger.getLogger(AppVersionCode.class.getName());
  
  public static final String PARAM_NAME = "versionCode";
...
LOG.log(Level.FINE, "Less than latest version: {0}, installation: {1}", new Object[]{ output,  installation});
}
...
if(LOG.isLoggable(Level.FINE)){
LOG.log(Level.FINE, "Less than or equals version {0}: {1}, installtion: {2}", new Object[]{ target,  output,  installation});
}
  ...
  public int get(HttpServletRequest request, int defaultValue) {
    int output;
    final String versionCode = request.getParameter(PARAM_NAME);  
    if(versionCode != null) {
