...
public class FileFeedBuilder extends BaseFeedCreator implements FilenameFilter {
    private transient static final Logger LOG = Logger.getLogger(FileFeedBuilder.class.getName());
  ...
  public Map<String, Feed> load(Path dirPath) {
    ...
    if(LOG.isLoggable(Level.FINE)){
      LOG.log(Level.FINE, "Dir: {0}, File names: {1}", 
new Object[]{ dirPath,  names == null ? null : Arrays.toString(names)});
    }
  ...
  public boolean updateFeed(Feed feed, Integer feedid, Path path) {
      ...
      try {
        ...
        if(LOG.isLoggable(Level.FINE)){
            LOG.log(Level.FINE, "Loading notice: {0}", path);
        }
        ...
        if(LOG.isLoggable(Level.WARNING)){
            LOG.log(Level.WARNING, "Error reading: " + path, e);
        }
        return false;
      }
  }
