...
--%>
        ...
        <jsp:useBean id="FeedSelectionBean" scope="session" class="com.idisc.web.beans.FutureFeedSelectionBean"/>

        <jsp:setProperty name="FeedSelectionBean" property="feed" value="${feed}"/>
        <jsp:setProperty name="FeedSelectionBean" property="request" value="<%=request%>"/>    
            
        <c:if test="${FeedSelectionBean.feeds != null && not empty FeedSelectionBean.feeds}">
            <small><i>
                <idisc:displayfeeds displayPageNav="false" displayPageLinks="false" pageSize="10"  
                                    feeds="${FeedSelectionBean.feeds}"/> 
            </i></small>
        </c:if>
    </jsp:attribute>
</idisc:page_with_slider>
